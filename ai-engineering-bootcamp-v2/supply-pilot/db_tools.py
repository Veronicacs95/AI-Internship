...
def save_agent_trace(trace: dict):
    """
    Save one completed SupplyPilot agent trace to PostgreSQL.

    The trace is generated by run_agent() and stored for
    observability and Week 4 evaluation.
    """
    ...
    with psycopg.connect(DATABASE_URL) as conn:
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                (
                    trace["user_input"],
                    json.dumps(trace["retrieved_context"], default=str),
